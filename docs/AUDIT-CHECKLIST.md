...
- ✅ Shell static checks cover `scripts/`, `recipes/`, `modules/`, and `archive/upstream-20260215/oneclick/` for all `*.sh`.
  - `archive/upstream-20260215/oneclick/` uses ShellCheck with severity gate `-S error` (upstream warning/info noise does not block CI, parse/syntax and error-level findings still block).
