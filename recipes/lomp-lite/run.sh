...
db_name="$(awk -F ':' '/^[[:space:]]+name:/ {gsub(/^[[:space:]]+|[[:space:]]+$/, "", $2); print $2; exit}' "${SITE_FILE}")"
db_user="$(awk -F ':' '/^[[:space:]]+user:/ {gsub(/^[[:space:]]+|[[:space:]]+$/, "", $2); print $2; exit}' "${SITE_FILE}")"
redis_ns="$(awk -F ':' '/^[[:space:]]+namespace:/ {gsub(/^[[:space:]]+|[[:space:]]+$/, "", $2); print $2; exit}' "${SITE_FILE}")"
...
echo "[INFO] plan.preflight: validate inventory references and topology"
echo "[INFO] plan.web: stage OLS and WP actions on host=${host_ref}"
echo "[INFO] plan.data: stage MariaDB and Redis actions on hub=${hub_ref}"
echo "[INFO] plan.site: stage db=${db_name} user=${db_user} redis_ns=${redis_ns}"
echo "[INFO] plan.ops: stage backup/restore/status/diagnostics workflows"
echo "[INFO] plan.rollback: keep previous configs and restore inventory snapshot"
